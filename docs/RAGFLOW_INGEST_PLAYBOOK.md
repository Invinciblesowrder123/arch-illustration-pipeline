...
| OpenAI-API-Compatible / VLLM | BAAI/bge-m3 | **硅基流动官方定价页列为「免费」**（输入输出均免）；另有付费的 Pro/BAAI/bge-m3 | 1024 | ★**迁移最省事**：与旧库本地 TEI 同模型，同维度时理论上可复用旧向量、免全量重解析 |
...
**关于「免费」**：bge-m3 那行的免费是硅基流动官方定价页当前的标注（2026-09 查证），
**不是合同保障**——平台有过把模型移出免费名单的先例，免费版也有限速，大批量入库仍要分批。
打算长期依赖的话，建议同时留一个付费备选。
