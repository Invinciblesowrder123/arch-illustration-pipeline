...
### 11.5 ★ 云端 bge-m3 与本地 TEI 向量等价 → **切换同模型不必重解析**（2026-09-26 实测）

原先的判断是"换 embedding 就必须全量重解析"。**对换模型成立，对换同一模型不成立**。
用 demo 库（本地 TEI bge-m3 旧向量，29 chunks）改指向云端 bge-m3 后检索，与同篇文档
新建的云端 bge-m3 库严格对照：

| 查询 | A 旧向量+云 query | B 云向量+云 query | 命中页码 |
|---|---|---|---|
| 南岛语族 扩散路径 动力机制 | 0.6581 | 0.6580 | [25,22,10] = [25,22,10] |
| 稻作农业 航海技术 传播 | 0.5588 | 0.5589 | [22,25,15] = [22,25,15] |
| 人口增长 资源压力 迁徙 | 0.5454 | 0.5455 | [21,13,24] = [21,13,24] |
| 独木舟 边架艇 远洋航行 | 0.3493 | 0.3493 | [16,23] = [16,23] |
| **平均 top1** | **0.5279** | **0.5279** | 差 **-0.0000** |

另外实测 `BAAI/bge-m3` 与 `Pro/BAAI/bge-m3` 对同一文本的余弦相似度 = **1.000000**
（同一模型，Pro 只是速率/计费档位不同，TPM 高一倍）。

**结论**：本地 TEI 与云端 bge-m3 是数值等价的。切换时**只改 dataset 的
`embedding_model` 引用即可，无需重解析** —— 5–8 小时压缩到约 1 分钟，且费用为 0（标准版免费）。

已应用到：考古文献库（165 篇 / 9971 chunks）、考古专著（21 / 2892）、demo 库、租户默认，
统一为 `BAAI/bge-m3@siliconflow@OpenAI-API-Compatible`。

**回退**：把 `embedding_model` 改回 `BAAI/bge-m3@Builtin` 并 `up --with-tei` 即可，向量本身没动过。
