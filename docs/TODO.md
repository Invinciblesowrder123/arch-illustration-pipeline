...
- [x] ✓ **自检纳入 RAGFlow 环境监测（2026-09-22）**
  `check.py` 新增 RAGFlow 探测项（配好 RAGFLOW_* 三项后自动生效，否则明确跳过）：
  鉴权 + dataset 存在性 + 一次真实检索（阈值 0，用于区分"通路故障"与"库为空"）。
  `requirements.txt` 注明 RAGFlow 模式仅依赖 requests（无需 ragflow-sdk）。
  顺带修复自检探测被 aixw 短输入风控拦截的缺陷（探针改真实任务 + 自动加长重试）。
  备注：真机验证通过（30 项单测全绿 + 实跑 --check）。
