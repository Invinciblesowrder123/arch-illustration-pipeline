...
自检开头会打印**环境指纹**与生效参数。换机器、或遇到"你这能跑我这不能跑"时，
两边各跑一次、逐行对比即可定位：

```text
环境指纹（两台机器可直接逐行对比）:
  - 项目版本: arch-illustration 1.1.0
  - Python: 3.13.14 (CPython) / Windows-11-10.0.22631-SP0
  - 关键库: openai=3.17.0, httpx=未安装, httpx2=2.13.0, requests=2.34.2, PyMuPDF=1.28.2, ...
生效的客户端参数（T5，来自 .env）: LLM_TIMEOUT=300s, LLM_TIMEOUT_KNOWLEDGE=600s,
  VISION_TIMEOUT=300s, IMG_TIMEOUT=300s, LLM_RETRIES=2, CHECK_TIMEOUT=180s
图内文字策略 TEXT_MODE=caption_only
```

> 注意那行 `httpx=未安装, httpx2=2.13.0`：openai **3.x 起改用 `httpx2`**，2.x 用的是 `httpx`
> ——两种组合都实测可用，本项目不直接依赖 httpx。精确组合见 `requirements.lock.txt`。
...
| `report.md` | 生成报告：知识层模式、检索词与**检索明细**、**引用文献列表（含页码）**、**图注表**、每轮问题清单与评分 |
| `illustration_attempt_N.png` | 各轮历史版本 |
| `caption_table.md` | 图注表（编号 ｜ 名称 ｜ 图上位置），`caption_only` 模式下是标注信息的正式载体 |
| `knowledge_summary.md` / `illustration_spec.json` | 该次运行的**知识快照**（修订历史 run 时用作基准） |

> `--revise` 产出的 `output/<run>_rev<N>/` 另有 `revision.json`（结构化记录：反馈原文、
> 结构化结果、逐条判定、累积的 `must_keep`），下一轮修订就以它为继承来源。
...
flowchart LR
    ...
    C --> E["知识学习 LLM<br/>知识摘要 + 绘图规格<br/>断言带 文献 p.X"]
    D --> E
    E --> F["提示词<br/>（含图内文字硬约束）"]
    F --> G["绘图模型"]
    G --> H["视觉模型审稿<br/>对照知识 + 图内文字判定"]
    H -- 未通过 -->|问题清单注入提示词| G
    H -- 通过 --> I["final_illustration.png<br/>+ report.md（引用列表 + 图注表）"]
    J["教授人眼反馈<br/>--feedback"] --> K["revise.py<br/>拆 must_change / must_keep"]
    K -->|以基准图为参考图重绘| G
    K -.->|逐条判定在审稿环节完成| H
```
...
- **提示词阶段**：把中/英文提示词按 `TEXT_MODE` 追加**图内文字硬约束**（中英双语各写一遍）。
- **校验阶段**：视觉模型读图，对照需求与知识摘要逐项核对，输出 JSON（通过与否 / 评分 /
  问题清单 / 建议）；未通过则把问题清单注入提示词自动重绘。
- **修订阶段（人工反馈）**：人看完图提一段意见 → `revise.py` 拆成
  `must_change`（必须改）与 `must_keep`（不许动）→ 以基准图为参考图重绘 →
  **逐条判定**（每条改了没有、证据在图面什么位置）→ 未落实的项并入下一轮。
- **原则**：**文献没写的细节宁可留白，不允许编造**——审稿提示词按同一约束执行；
  三种降级（反馈拆分失败 / 上游不支持送参考图 / 校验漏判某条）**都必须写进报告**，不静默放行。
...
- **`--revise` 报"没有 final_illustration.png"**：那次运行当时**没通过**校验，没有成品可作基准。
  改用 `--refine-from last`（用最后一张尝试图），或直接指定文件名。
- **修订报告里出现"未判定"**：校验模型这一轮没对某条给判定。系统按**保守处理计为不通过**
  （宁可多跑一轮，也不静默放行）。可加大 `--max-attempts`，或把 `VISION_MODEL` 换成更稳的读图模型。
- **修订报告写"已退化为纯提示词重绘"**：上游不支持图片编辑接口（`images.edit`）。
  结果仍可用，但**漂移风险更高**——报告里会注明；想避免就换支持图生图的服务商。
- **图内还是出现英文/乱码**：说明这一轮校验判不通过并要求重画。若反复出现，
  改用 `--text-mode caption_only`（图内不写字，名称放图注表）最稳。
- **想让图面上保留"1、2、3"这样的编号数字**：默认的 `caption_only` 是**零文字（含数字）**，
  编号与名称都在图注表里。要图内可见就用 `--text-mode in_image`。
- **换机器后"你这能跑我这不能跑"**：两边各跑 `python main.py --check --skip-image`，
  对照打印的环境指纹；精确依赖组合见 `requirements.lock.txt`。
...
- **修订的逐条判定宁严勿松**：`must_change` 判为"部分满足"也计为未通过（最多跑 `--max-attempts` 轮）
  ——这是为了不让"只写不查"变成静默放行；代价是可能多花一张图。
- **`caption_only` 的"图内零文字"包含阿拉伯数字**（按任务书执行）。图注表用
  「编号 ｜ 名称 ｜ 图上位置」对应图面引线；若希望图内保留编号数字，需改这一条判定规则。
- **图生图依赖上游支持 `images.edit`**，不支持时自动降级为纯提示词重绘（报告中注明降级原因）。
- 修订**默认不重新检索**（避免上下文漂移）；反馈若指向"文献依据有问题"，请显式加 `--re-retrieve`。
- 链式修订的 `must_keep` 会**累积继承**——保留项只会越来越多，跑几轮后建议人工清一遍
  （重新起一次以某版为基准的修订，不要无限链下去）。
