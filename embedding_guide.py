...
PROVIDER_RECOMMENDATIONS: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "dim": "1024",
        "when": "★ 迁移最省事（**2026-09-26 已实测通过**）：与旧库本地 TEI 同一个模型，"
                "向量空间数值等价（4 组查询相似度差 ≤0.0001、命中页码完全一致），"
                "**改引用即可，免全量重解析**——5–8 小时压缩到 1 分钟。通过 OpenAI 兼容接口接入。\n"
                "     ⚠ 标价免费 ≠ 开箱可用：2026-09 实测，未实名或账户余额为负时，"
                "连免费模型也直接 402 `account balance is insufficient`（平台对推理调用统一校验余额）。"
                "需先在控制台完成实名认证，并让余额不为负（最低充值 ¥10；实名用户可领 ¥16 代金券）。\n"
                "     ⚠ 「免费」也不是合同保障：有过模型被移出免费名单的先例，免费版限速固定，"
                "大批量入库仍要分批。",
